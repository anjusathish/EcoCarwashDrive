 ...
 extension Date
 {
     ...
     func firstDayOfTheMonth() -> Date {
         return Calendar.current.date(from: Calendar.current.dateComponents([.year,.month,.hour,.month,.second,.timeZone], from: self))!
     }
     
     func getAllDays() -> [Date]
     {
         let days = firstDayOfTheMonth().daysOfWeek(using: .gregorian)
